...
| Timeline persistence resilience | ADR-002 | `ChangeEventAdapter.java`, `ProcessDeployResultService.java` (try-catch + counter) | `ProcessDeployResultServiceTest` | `timeline_persistence_failures_total{service="<service>"}` | [Link] |
...
| Test automation scripts | N/A | `tests/run_unit_tests.sh`, `tests/run_integration_tests.sh`, `tests/run_automated_manual_tests.sh` (CT-02..CT-32, CT-SEC-*), `tests/run_rate_tests.sh` | CT-02 through CT-SEC-10 | N/A | [Link] |