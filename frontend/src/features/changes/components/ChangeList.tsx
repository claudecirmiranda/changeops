  ...
  if (error && changes.length === 0) {
    return (
      <div className="rounded-xl bg-red-50 border border-red-200 px-4 py-4 text-sm text-red-700 flex items-center gap-3 shadow-sm">
        <svg width="20" height="20" style={{flexShrink:0}} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" /></svg>
        <span>Failed to load changes: {error.detail}</span>
        <button onClick={load} className="ml-auto text-red-600 underline underline-offset-2 hover:text-red-800 font-medium text-xs">Retry</button>
      </div>
    )
  }
    ...
    <div>
      ...
      {pollError && (
        <div className="mb-3 rounded-lg bg-amber-50 border border-amber-200 px-3 py-2.5 text-xs text-amber-800 flex items-center gap-2">
          <svg width="16" height="16" style={{flexShrink:0}} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" /></svg>
      ...
      {changes.length === 0 ? (
        <div className="text-center py-20 rounded-xl border border-dashed border-slate-300 bg-white">
          <svg width="48" height="48" style={{margin:'0 auto 1rem',color:'#cbd5e1'}} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 00.75-.75 2.25 2.25 0 00-.1-.664m-5.8 0A2.251 2.251 0 0113.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25zM6.75 12h.008v.008H6.75V12zm0 3h.008v.008H6.75V15zm0 3h.008v.008H6.75V18z" />
          </svg>
          <p className="text-base font-medium text-slate-500">No changes registered yet</p>
          <p className="text-sm text-slate-400 mt-1">Create your first change using the button above.</p>
        </div>
      ) : (
      ...
      {page && page.totalPages > 1 && (
        <div className="mt-4 flex items-center justify-between text-sm text-slate-500">
          <span className="text-xs">
            {page.totalElements} changes &mdash; Page {page.number + 1} of {page.totalPages}
          </span>
  ...
  return (
    <div className="rounded-xl border border-slate-200 overflow-hidden shadow-sm animate-pulse">
      ...
      {Array.from({ length: 5 }).map((_, i) => (
        <div key={i} className={`h-14 border-b border-slate-50 flex items-center px-4 gap-4 ${i % 2 !== 0 ? 'bg-slate-50/40' : 'bg-white'}`}>
