    ...
    <div className="rounded-xl border border-slate-200 shadow-sm bg-white overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-slate-800 to-indigo-800 px-5 py-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-indigo-300/80 font-mono mt-0.5 truncate max-w-xs">{changeId}</p>
        </div>
      ...
      <div className="px-6 py-6">
        ...
        {error && (
          <div className="flex items-center gap-2 text-sm text-red-600">
            <svg width="16" height="16" style={{flexShrink:0}} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" /></svg>
        ...
        {!loading && !error && events.length === 0 && (
          <div className="text-center py-8">
            <svg width="40" height="40" style={{margin:'0 auto 0.75rem',color:'#e2e8f0'}} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
        ...
        {!loading && events.length > 0 && (
          <ol className="relative border-l-2 border-slate-100 space-y-6 ml-3">
            {events.map((event) => {
              const dot = dotColor[event.eventType] ?? 'bg-slate-400 ring-slate-200'
              const icon = iconBg[event.eventType] ?? 'bg-slate-100 text-slate-500'
                  ...
                  <div className="rounded-lg border border-slate-100 bg-slate-50/60 px-4 py-3">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex items-center gap-2.5 min-w-0">
                        <span className={`inline-flex w-7 h-7 items-center justify-center rounded-full shrink-0 ${icon}`}>
                    ...
                    {parsed && (
                      <details className="mt-2">
                        ...
                        <pre className="mt-2 ml-9 overflow-x-auto rounded-lg bg-white border border-slate-100
