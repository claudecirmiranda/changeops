        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="w-10 h-10 rounded-xl bg-indigo-500 flex items-center justify-center shadow-lg">
              <span className="text-white text-xs font-bold tracking-wider">CO</span>
            </div>
            <div>
              <h1 className="text-lg font-bold text-white tracking-tight">ChangeOps</h1>
              <p className="text-xs text-indigo-300/80 mt-0.5">Change Management Dashboard</p>
            </div>
          </div>
          <button
        ...
        {successMsg && (
          <div className="rounded-xl bg-emerald-50 border border-emerald-200 px-4 py-3 text-sm text-emerald-800 flex items-center justify-between shadow-sm">
            <span className="flex items-center gap-2">
              <svg width="16" height="16" style={{color:'#10b981',flexShrink:0}} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
              {successMsg}
            </span>
            <button onClick={() => setSuccessMsg(null)} className="text-emerald-400 hover:text-emerald-700 text-xl leading-none ml-4">×</button>
          </div>
        )}
...
function StatCard({
  label, value, valueClass, bg, border,
}: { label: string; value: number; valueClass: string; bg: string; border: string }) {
  return (
